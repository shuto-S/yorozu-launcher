...
/// Shares only bounded ownership generations between the main-actor writer and
/// the monitor actor. Clipboard contents never enter this tracking state.
final class ClipboardOwnedChanges: Sendable {
    private static let maximumCount = 16
    private let generations = Mutex<[Int]>([])

    func record(_ changeCount: Int) {
        generations.withLock { values in
            guard !values.contains(changeCount) else { return }
            values.append(changeCount)
            if values.count > Self.maximumCount { values.removeFirst() }
        }
    }

    func contains(_ changeCount: Int) -> Bool {
        generations.withLock { $0.contains(changeCount) }
    }
}
...
final class SystemPasteboardReader {
    ...
    private let sourceApplication: () -> (bundleIdentifier: String?, localizedName: String?)

    init(
        pasteboard: NSPasteboard = .general,
        sourceApplication: @escaping () -> (bundleIdentifier: String?, localizedName: String?) = {
    ...
    func readSnapshot(
        ...
        // The monitor's count was read on a separate actor hop. Do not read a
        // newer clipboard (including our own temporary write or restoration).
        guard pasteboard.changeCount == expectedChangeCount,
              !ownedChanges.contains(expectedChangeCount) else { return nil }
        ...
        // Lazy pasteboard providers can change ownership while providing data.
        guard pasteboard.changeCount == expectedChangeCount,
              !ownedChanges.contains(expectedChangeCount) else { return nil }
...
actor ClipboardMonitor {
    nonisolated let ownedChanges = ClipboardOwnedChanges()
    ...
    func poll(settings: ClipboardRecordingSettings) async {
        let changeCount = await reader.changeCount
        guard changeCount != lastChangeCount else { return }
        lastChangeCount = changeCount
        guard !ownedChanges.contains(changeCount) else { return }
...
final class SystemPasteboardAccessor: PasteboardAccessing {
    ...
    private let readData: (NSPasteboardItem, NSPasteboard.PasteboardType) -> Data?
    private let ownedChanges: ClipboardOwnedChanges?

    init(
        pasteboard: NSPasteboard = .general,
        writeObjects: (([any NSPasteboardWriting]) -> Bool)? = nil,
        readData: ((NSPasteboardItem, NSPasteboard.PasteboardType) -> Data?)? = nil,
        ownedChanges: ClipboardOwnedChanges? = nil
    ) {
        self.pasteboard = pasteboard
        self.writeObjects = writeObjects ?? { pasteboard.writeObjects($0) }
        self.readData = readData ?? { $0.data(forType: $1) }
        self.ownedChanges = ownedChanges
    }
    ...
    func snapshot() -> PasteboardSnapshotResult {
        ...
        for item in pasteboardItems {
            ...
            for type in item.types {
                // A promised representation can be unavailable while another
                // remains readable. Restoring only that subset would lose data.
                guard let data = readData(item, type) else { return .unavailable }
    ...
    func replace(
        ...
        guard pasteboard.changeCount == expectedChangeCount else { return .clipboardChanged }
        let ownershipChangeCount = pasteboard.clearContents()
        ownedChanges?.record(ownershipChangeCount)
        let didWrite: Bool
        switch prepared {
        case let .items(items):
            didWrite = writeObjects(items)
        }
        // writeObjects adds representations to our ownership generation. If
        // another owner copied during the write, neither report its content as
        // ours nor roll it back to the snapshot taken before our operation.
        guard pasteboard.changeCount == ownershipChangeCount else { return .clipboardChanged }
        guard didWrite else {
            return restore(snapshot)
                ? .writeFailedAndRestored
                : .writeFailedAndRestoreFailed
        }
        return .written(changeCount: ownershipChangeCount)
    }
...
final class PasteCoordinator {
    ...
    init(monitor: ClipboardMonitor) {
        pasteboard = SystemPasteboardAccessor(ownedChanges: monitor.ownedChanges)
        // Production excludes our exact write generations, not a time window
        // that also drops the user's next copy. Keep the hook for injected tests.
    ...
    func copy(_ content: PasteboardContent) async -> PasteboardReplacementResult {
        guard !isOperationInProgress else { return .busy }
        ...
        // The injected hook can suspend. A copy made during that hop belongs
        // to the user and must not be replaced or used as an old backup.
        guard pasteboard.changeCount == originalChangeCount else { return .clipboardChanged }
    ...
    func performPaste(
        ...
        guard case let .written(injectedChangeCount) = replacement else {
            ...
            case .preservationLimitExceeded, .preservationFailed:
                return .failedBecauseClipboardCouldNotBePreserved
        ...
        let activationResult = await waitForActivation(of: targetApplication)
        guard pasteboard.changeCount == injectedChangeCount else { return .clipboardChanged }
        ...
        // The shortcut pastes whatever is currently on the system clipboard,
        // not the content argument. Do not send another application's new copy.
        guard pasteboard.changeCount == injectedChangeCount else { return .clipboardChanged }
        guard !targetApplication.isTerminated else {
            return .copiedBecauseTargetUnavailable
        }
        ...
        if pasteboard.changeCount == injectedChangeCount {
            await suppressClipboardMonitor(.seconds(2))
            guard pasteboard.changeCount == injectedChangeCount else { return .pasted }
            guard pasteboard.restore(original) else { return .pastedButClipboardRestoreFailed }
        }
        return .pasted
    }
